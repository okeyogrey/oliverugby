...
<section class="py-12 bg-gray-50">
    <div class="container mx-auto px-6">
        ...
        <!-- Current Players -->
        <div id="current-players" class="grid md:grid-cols-3 gap-8">
            @foreach($currentPlayers as $player)
                <div class="bg-white rounded-xl shadow hover:shadow-lg transition overflow-hidden cursor-pointer"
                    ...
                    <div class="relative">
                        <img src="{{ asset('storage/' . $player->photo) }}" class="w-full h-56 object-cover">
                    ...
                    <div class="p-5 text-center">
                        ...
                        <p class="text-xs text-gray-500">{{ $player->age }}years | {{ $player->height }}cm | {{ $player->weight }}kg</p>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <!-- Past Players -->
        <div id="past-players" class="grid md:grid-cols-3 gap-8 hidden">
            @foreach($pastPlayers as $player)
                <div class="bg-white rounded-xl shadow hover:shadow-lg transition overflow-hidden cursor-pointer"
                    ...
                    <div class="p-5 text-center">
                        <h3 class="text-lg font-bold">{{ $player->name }}</h3>
                        <p class="text-sm text-gray-600">{{ $player->position }}</p>
                        <p class="text-xs text-gray-500">Age: {{ $player->age }} | {{ $player->height }}cm | {{ $player->weight }}kg</p>
                    </div>
                </div>
            @endforeach
        </div>
